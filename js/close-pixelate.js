...
// don't proceed if canvas is no supported
  ...
  ClosePixelation.prototype.render = function (options) {
    ...
    // set size
    var w = this.width = this.canvas.width = 64
    var h = this.height = this.canvas.height = 64
  ...
  ClosePixelation.prototype.renderClosePixels = function (opts) {
    ...
    var row, col, x, y, pixelY, pixelX, pixelIndex, red, green, blue, pixelAlpha, intercept
    ...
    for (row = 0; row < rows; row++) {
      ...
      for (col = 0; col < cols; col++) {
        ...
        red = red * contrast + intercept
        green = green * contrast + intercept
        blue = blue * contrast + intercept

        red = Math.round(Math.max(30, Math.min(red, 255)))
        green = Math.round(Math.max(30, Math.min(green, 255)))
        blue = Math.round(Math.max(30, Math.min(blue, 255)))
